...
<div class="table-margin">
    <table width="100%" class="table table-bordered table-header">
        <thead>
            ...
            <th width="50" style="text-align:center"><?php echo L('listorder');?></th>
...
<tbody>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
		...
		<input type="button" class="btn btn-mystyle btn-sm" value="<?php echo L('push');?>" onclick="push();"/>
		<?php if($workflow_menu) { ?><input type="button" class="btn btn-mystyle btn-sm" value="<?php echo L('reject');?>" onclick="reject_check()"/>
		...
		<table cellpadding='0' cellspacing='1' border='0'><tr><tr><td colspan='2'><textarea name='reject_c' id='reject_c' style='width:300px;height:46px;'  onfocus="if(this.value == this.defaultValue) this.value = ''" onblur="if(this.value.replace(' ','') == '') this.value = this.defaultValue;"><?php echo L('reject_msg');?></textarea></td><td><input type='button' value=' <?php echo L('submit');?> ' class="btn btn-mystyle btn-sm" onclick="reject_check(1)"></td></tr>
		</table>
		</div>
		<?php }}?>
		<input type="button" class="btn btn-mystyle btn-sm" value="<?php echo L('remove');?>" onclick="myform.action='?m=content&c=content&a=remove&catid=<?php echo $catid;?>&menu_id=<?php echo $_GET['menu_id'];?>';myform.submit();"/>
