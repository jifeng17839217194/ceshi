...
<div class="page-header">
	<div class="pull-left">
		<h4><?php echo L('model_manage');?>--<?php echo $r['name'];?><?php echo L('field_manage');?></h4>      
	</div>
</div>
<div class="subnav">

<div class="content-menu ib-a blue line-x"><a class="add fb" href="?m=content&c=sitemodel_field&a=add&modelid=<?php echo $modelid?>&menu_id=<?php echo $_GET['menu_id']?>"><em><?php echo L('add_field');?></em></a>
　<a class="on" href="?m=content&c=sitemodel_field&a=init&modelid=<?php echo $modelid?>&menu_id=<?php echo $_GET['menu_id']?>"><em><?php echo L('manage_field');?></em></a><span>|</span><a href="?m=content&c=sitemodel_field&a=public_priview&modelid=<?php echo $modelid?>&menu_id=<?php echo $_GET['menu_id']?>" target="_blank"><em><?php echo L('priview_modelfield');?></em></a>
</div></div>
<div class="pad-lr-10">
...
<div class="table-list">
    ...
    <tr>
		...
		<td align='center'> <a href="?m=content&c=sitemodel_field&a=edit&modelid=<?php echo $r['modelid']?>&fieldid=<?php echo $r['fieldid']?>&menu_id=<?php echo $_GET['menu_id']?>"><?php echo L('edit');?></a> | 
		<?php if(!in_array($r['field'],$forbid_fields)) { ?>
		<a href="?m=content&c=sitemodel_field&a=disabled&disabled=<?php echo $r['disabled'];?>&modelid=<?php echo $r['modelid']?>&fieldid=<?php echo $r['fieldid']?>&fieldid=<?php echo $r['fieldid']?>&menu_id=<?php echo $_GET['menu_id']?>"><?php echo $r['disabled'] ? L('field_enabled') : L('field_disabled');?></a>
		<?php } else { ?><font color="#BEBEBE"> <?php echo L('field_disabled');?> </font><?php } ?>|<?php if(!in_array($r['field'],$forbid_delete)) {?> 
		<a href="javascript:confirmurl('?m=content&c=sitemodel_field&a=delete&modelid=<?php echo $r['modelid']?>&fieldid=<?php echo $r['fieldid']?>&menu_id=<?php echo $_GET['menu_id']?>','<?php echo L('confirm',array('message'=>$r['name']))?>')"><?php echo L('delete')?></a><?php } else {?><font color="#BEBEBE"> <?php echo L('delete');?></font><?php }?> </td>
	</tr>
	<?php } ?>
    </tbody>
    </table>
