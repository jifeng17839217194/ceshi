...
class content extends admin {
	...
	public function __construct() {
		...
		//权限判断
		//权限判断合并至主权限表k_admin_role_priv by zhenye 19/5/31
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			if($from=='train'){
				$datalink = getcache(3364,'linkage');
				$infos = $datalink['data'];
				
				if(isset($_GET['beetype-3']) && !empty($_GET['beetype-3'])) {
					$beetype =intval($_GET['beetype-3']);
					$where .= " AND `beetype` = '$beetype'";
				}elseif(isset($_GET['beetype-2']) && !empty($_GET['beetype-2'])){
					$beetype =intval($_GET['beetype-2']);
					$where .= " AND `beetype` in(".$infos[$beetype]['arrchildid'].")";
				}elseif(isset($_GET['beetype-1']) && !empty($_GET['beetype-1'])){
					$beetype =intval($_GET['beetype-1']);
					// print_r($infos[$beetype][arrchildid]);
					$where .= " AND `beetype` in(".$infos[$beetype]['arrchildid'].")";
				}
			}
			...
			if(!$from){
				...
				include $this->admin_tpl('content_list_train');
			}
	...
	public function add() {
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($catid==109 && $_POST['beetype-3']=='') showmessage('课程分类为必选项，请选择至三级');
