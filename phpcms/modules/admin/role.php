...
class role extends admin {
	...
	public function role_priv() {
		...
		if(isset($_POST['dosubmit'])){
			...
			menu_op::updateMenu($_POST['roleid']);
			...
			if ($siteid) {
				...
				$menu->icon = array('│ ','├─ ','└─ ');
				$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
				$result = \think\Db::name('menu')->where('status',1)->where('type','<>','2')->select();
				...
				foreach ($result as $n=>$t) {
					...
					$result[$n]['parentid_node'] .= ($result[$n]['level'] <= 1)? ' data-initialize="true"' : '';
					//$result[$n]['parentid_node'] .= ($t['id'] == 102 || $t['parentid'] == 102)? ' data-hidden="true"' : '';
				}
				$str  = "<tr id='node-\$id' \$parentid_node style='text-align: left;'>
							<td style='padding-left:30px;text-align: left;'>\$spacer<input type='checkbox' name='menuid[]' value='\$id' level='\$level' \$checked onclick='javascript:checknode(this);'> \$cname</td>
						</tr>";
	 ...
	 * 设置菜单权限
	 */
	public function setting_adminmenu() {
	    $roleid = isset($_GET['roleid']) && intval($_GET['roleid']) ? intval($_GET['roleid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
	    ...
	    include $this->admin_tpl('role_adminmenu');
