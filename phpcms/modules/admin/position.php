...
class position extends admin {
	...
	public function public_item() {
		...
		if(isset($_GET['dosubmit'])) {
			...
			$items = count($_POST['items']) > 0  ? $_POST['items'] : showmessage('您没有勾选信息',HTTP_REFERER);
	...
	public function public_item_manage() {
		if(isset($_POST['dosubmit'])) {
			...
			$array['inputtime'] = $pos_arr['inputtime'] ? $pos_arr['inputtime'] : SYS_TIME;
			...
			$array = array('data'=>array2string($array),'imgurl'=>trim($_POST['info']['imgurl']),'synedit'=>intval($_POST['synedit']),'thumb'=>$thumb);
	 ...
	 * 推荐位文章停用、启用
	 */
	public function public_item_start() {
		if(isset($_POST['posid'])) {
			$status=$_GET['status'];
			if(empty($_POST['items'])) showmessage('您没有勾选信息');
			if(is_array($_POST['items'])) {
				$sql = array();
				foreach ($_POST['items'] as $item) {
					$pos = explode('-', $item);
					// print_r($pos);
					$this->db_data->update(array('status'=>$status),array('id'=>$pos[0],'modelid'=>$pos[1],'posid'=>$_POST['posid']));
					// echo $this->db_data->lastsql();
				}
			}
			showmessage(L('operation_success'),HTTP_REFERER);
		} else {
