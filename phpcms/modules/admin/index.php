...
class index extends admin {
	...
	// 问题类型数据量
	public function cnt_wentype(){
		$temp_data=GetCurl(API_PATH.'getwenmodulerank');
		...
		foreach($temp as $key2=>$val2){
			$tempData['name'][]=$val2['name'];
			$tempData['numbers'][]=$val2['numbers'];
		}
		$title='问题类型数据量';
		include $this->admin_tpl('cnt_info');
	}
	// 专家回复排行榜（TOP20）
	public function cnt_expertanswer(){
		$temp_data=GetCurl(API_PATH.'getexpertrank');
		$tempArray= json_decode($temp_data,true);
		...
		include $this->admin_tpl('cnt_getexpertrank');
	}
	// 资讯点赞（TOP100）
	public function cnt_info(){
		$temp_data=GetCurl(API_PATH.'getnewsrank');
		$tempArray= json_decode($temp_data,true);
		...
		include $this->admin_tpl('cnt_getnewsrank');
	}
	//收藏最多课时
	public function cnt_train(){
		$temp_data=GetCurl(API_PATH.'getfavoriterank');
		$tempArray= json_decode($temp_data,true);
		...
		include $this->admin_tpl('cnt_getfavoriterank');
	}
	...
	public function public_main() {
		...
		//增加相关数据统计
		// $sys_cache['web_count_uid']=$this->userdb->count();//用户
		// $usewhere=' regdate > '.strtotime(date('Y-m-d',SYS_TIME));
		// $sys_cache['new_count_uid']=$this->userdb->count($usewhere);//用户新增
		
		// $sys_cache['web_count_thread']=$this->wendb->count();//提问
		// $wenwhere=' dateline > '.strtotime(date('Y-m-d',SYS_TIME));
		// $sys_cache['new_count_thread']=$this->wendb->count($wenwhere);;
		
		// $sys_cache['web_count_post']=$this->postdb->count();//回复
		// $sys_cache['new_count_post']=$this->postdb->count($wenwhere);
		// $this->expertdb = pc_base::load_model('member_verify_model');
		...
		// foreach($grouplist as $v){
			...
			// }
		// }
		// $expertwhere='usertype in('.implode(',',$arrg).')';
		// $sys_cache['web_count_expert']=$this->userdb->count($expertwhere);//专家
		// $whereexpcheck=' status =0 ';
		// $sys_cache['new_count_experapply']=$this->expertdb->count($whereexpcheck);
		// $modelid=16;
		// $this->cntdb = pc_base::load_model('content_model');
		// $this->cntdb->set_model($modelid);
		// $sys_cache['web_count_video']=$this->cntdb->count();//课程
		// $videowhere='inputtime > '.strtotime(date('Y-m-d',SYS_TIME));
		// $sys_cache['new_count_video']=$this->cntdb->count($videowhere);
	...
	public function init () {
		...
		//增加相关数据统计
		$usewhere=' status =99';
		$sys_cache['web_count_uid']=$this->userdb->count($usewhere);//用户
		$usewhere.=' AND regdate > '.strtotime(date('Y-m-d',SYS_TIME));
		$sys_cache['new_count_uid']=$this->userdb->count($usewhere);//用户新增
		$sys_cache['web_count_thread']=$this->wendb->count();//提问
		$wenwhere=' dateline > '.strtotime(date('Y-m-d',SYS_TIME));
		$sys_cache['new_count_thread']=$this->wendb->count($wenwhere);;
		...
		// print_r($arrg);
		$expertwhere='status=99 AND usertype in(9,10,11)';
		$sys_cache['web_count_expert']=$this->userdb->count($expertwhere);//专家
		$whereexpcheck=' status =0 ';
		$sys_cache['new_count_experapply']=$this->expertdb->count($whereexpcheck);
		$modelid=16;
		...
		$sys_cache['web_count_comment']=$this->cmtdb->count();//课程评论
		$cmtwhere='creat_at > '.strtotime(date('Y-m-d',SYS_TIME));
		$sys_cache['web_new_comment']=$this->cmtdb->count($cmtwhere);
		//统计数据结束
