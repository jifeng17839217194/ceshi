...
class admin {
	...
	private function getTopId($id){
	    $r = Db::name('menu')->where('id',$id)->field('id,parentid')->find();
	    if(!empty($r) && $r['parentid'] != '0') return $this->getTopId($r['parentid']);
	    return $r['id'];
	}
	
	private function create_admin_menu(){
	    $roleid = $_SESSION['roleid'];
	    $this->menuId = intval($_GET['menu_id']);
	    pc_base::load_app_class('menu_op','admin',0);
	    $this->menuList = menu_op::getMenu($roleid);
	    $this->menuTopId = 101;
	    $this->menuCurrent = Db::name('menu')->find($this->menuId);
	    if(!empty($this->menuCurrent)){
	        $this->menuTopId = $this->getTopId($this->menuId);
	    }
	}
	...
	final public function check_priv() {
		...
		$result = Db::name('admin_role_priv')->where(['m'=>ROUTE_M,'c'=>ROUTE_C,'a'=>$action,'roleid'=>$_SESSION['roleid']])->column('data');
		if(empty($result)) showmessage('您没有权限操作该项!',HTTP_REFERER);
		...
		if(!$checkSign){
		    showmessage('您没有权限操作该项!',HTTP_REFERER);
		}
	}
