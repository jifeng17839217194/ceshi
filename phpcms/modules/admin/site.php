...
class site extends admin {
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$telephone = isset($_POST['telephone']) && trim($_POST['telephone']) ? trim($_POST['telephone']) : '';
			...
			if ($this->db->insert(array('name'=>$name,'dirname'=>$dirname,'qq'=>$qq,'telephone'=>$telephone, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style))) {
	...
	public function edit() {
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			if (isset($_POST['dosubmit'])) {
				...
				$setting = trim(array2string($_POST['setting']));
				$sql = array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain,'qq'=>$qq,'telephone'=>$telephone, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template, 'setting'=>$setting, 'default_style'=>$default_style);
