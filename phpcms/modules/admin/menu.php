...
class menu extends admin {
	...
	public function child(){
	    ...
	    if($roleid == 1){
	        $first = \think\Db::name('menu')->where('id',$menuid)->find();
	        $pid = $first['id'];
	        if($first['parentid']==0){
	            $second = \think\Db::name('menu')->where('parentid',$menuid)->where('status',1)->where('type','>',0)->order('listorder asc,id asc')->find();
	            if(!empty($second)){
    	            $pid = $second['id'];
	            }
	        }
	        $info = \think\Db::name('menu')->where('parentid',$pid)->where('status',1)->where('type','>',0)->order('listorder asc,id asc')->find();
	    }
	    else{
	        $first = \think\Db::name('menu')->where('id',$menuid)->find();
	        $pid = $first['id'];
	        if($first['parentid']==0){
	            $secondlist = \think\Db::name('menu')->where('parentid',$menuid)->where('status',1)->where('type',1)->order('listorder asc,id asc')->select();
	            foreach ($secondlist as $k=>$v){
	                $r = \think\Db::name('admin_role_priv')->where(['m'=>$v['m'],'c'=>$v['c'],'a'=>$v['a'],'data'=>$v['data'],'roleid'=>$roleid])->find();
	                if($r){
	                    $pid = $v['id'];
	                    break;
	                }
	        ...
	        $list = \think\Db::name('menu')->where('parentid',$pid)->where('status',1)->where('type',1)->order('listorder asc,id asc')->select();
	        foreach ($list as $k=>$v){
	            $r = \think\Db::name('admin_role_priv')->where(['m'=>$v['m'],'c'=>$v['c'],'a'=>$v['a'],'data'=>$v['data'],'roleid'=>$roleid])->find();
	            if($r){
	                $info = $v;
	    ...
	    $args['c'] = $info['c'];
	    $args['a'] = $info['a'];
	    $args['menu_id'] = $info['id'];
	    $args['pc_hash'] = $_SESSION['pc_hash'];
	    parse_str($info['data'],$data);
	    if(!empty($data)){
