...
class linkage extends admin {
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=linkage&a=add\', title:\''.L('linkage_add').'\', width:\'600\', height:\'300\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('linkage_add'));
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$info['image'] = trim($_POST['info']['image']);
			$this->db->insert($info);
	...
	public function public_manage_submenu() {
		...
		foreach($result as $areaid => $area){
			...
			$areas[$area['linkageid']]['str_manage'] = ($sum > 40 && $this->_is_last_node($area['keyid'],$area['linkageid'])) ? '<a href="?m=admin&c=linkage&a=public_manage_submenu&keyid='.$area['keyid'].'&parentid='.$area['linkageid'].'&menu_id='.$_GET['menu_id'].'">'.L('linkage_manage_submenu').'</a> | ' : '';
			$areas[$area['linkageid']]['str_manage'] .= '<a href="javascript:void(0);" onclick="add(\''.$keyid.'\',\''.new_addslashes($area['name']).'\',\''.$area['linkageid'].'\')">'.L('linkage_add_submenu').'</a> | <a href="javascript:void(0);" onclick="edit(\''.$area['linkageid'].'\',\''.$area['name'].'\',\''.$area['parentid'].'\')">'.L('edit').'</a>';
			if(($keyid =='3364' && !in_array($area['linkageid'],array(3366,3367,3368))) || $keyid !='3364'){
			$areas[$area['linkageid']]['str_manage'] .='| <a href="javascript:confirmurl(\'?m=admin&c=linkage&a=delete&linkageid='.$area['linkageid'].'&keyid='.$area['keyid'].'\', \''.L('linkage_is_del').'\')">'.L('delete').'</a> ';
			}
		...
		$str  = "<tr>
					...
					<td >\$description</td>
					<td style='text-align:left;'>\$str_manage</td>
				</tr>";
		$tree->init($areas);
		$submenu = $tree->get_tree($sql_parentid, $str);
		//$big_menu =array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=admin&c=linkage&a=public_sub_add&keyid='.$keyid.'\', title:\''.L('linkage_add').'\', width:\'600\', height:\'430\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('linkage_add'));		
	...
	public function public_sub_add() {		
		if(isset($_POST['dosubmit'])) {
			...
			$info['image'] = trim(intval($_POST['info']['image']));
			...
			$list = form::select_linkage($keyid,'0','info[parentid]', 'parentid', L('cat_empty'), $linkageid,' onchange="DispayPrice()"');
			$show_validator = true;
	...
	//判断层级
	public function getReturnDegree(){
		$keyid = intval($keyid);
		$linkageid = $_GET['linkageid'];
		$return_arr = array();
   
		$result = $this->db->get_one(array('linkageid'=>$linkageid));
		if($result['parentid']==0){
			 $return_arr['status'] = 1;
			// return 1;
		}elseif($result['child']==1){
			 $return_arr['status'] = 2;
			// return 2;
		}
		echo json_encode($this->gbk2utf8($return_arr));
	}
	
	/**
	 * 数组转json字符串，中文要GBK转码utf8
	 */
	private function gbk2utf8($data)
	{
		if(is_array($data))
		{
