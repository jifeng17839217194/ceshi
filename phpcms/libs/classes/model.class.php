...
class model {
	 ...
	 * @param $limit
	 */
	final public function select_sql($data = '',$sql='',$order='',$limit='') {
	    return $this->db->select_sql($data, $sql, $order, $limit);
	}
	 ...
	 * @return unknown_type
	 */
	final public function mylistinfo($data = '',$sql='',$order='', $page = 1, $pagesize = 20, $setpages = 10,$urlrule = '',$array = array()) {
			$this->number = $this->db->count_sql($sql);
			$page = max(intval($page), 1);
			$offset = $pagesize*($page-1);
			$this->pages = pages($this->number, $page, $pagesize, $urlrule, $array, $setpages);
			$array = array();
			if ($this->number > 0) {
					return $this->db->select_sql($data, $sql, $order, "$offset, $pagesize");
			} else {
					return array();
			}
	}
