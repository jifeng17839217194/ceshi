...
function return_firstpy($tag)
{
	...
	if(is_array($pinyin)) {
		foreach($pinyin as $v){
		  $Farray[]=strtoupper(substr(preg_replace('/[^0-9a-zA-Z]/', '', $v),0,1));
	  }
	  $arraydata = implode('', $Farray);
	}
	 return $arraydata;
}
/**
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page<$pages) {
			...
			$multipage .= '<li class="active"> <span>'.$pages.'</span></li><li> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a></li>';
		} else {
			$multipage .= '<li> <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
		}
	}
